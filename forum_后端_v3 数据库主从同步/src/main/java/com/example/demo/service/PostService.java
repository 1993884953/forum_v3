...
//    @Cacheable(cacheNames = "categories", key = "'id:'")
    public List<Category> categories() {
        ...
        Cache.ValueWrapper value = cache.get("");
        if(value!=null) return (List<Category>) value.get();
...
//    @Cacheable(cacheNames = "categories", key = "'id:' +#id")
    public String categoryId(int id) {
        Cache cache = cacheManager.getCache("categories");
        Cache.ValueWrapper value = cache.get("id:"+id);
        System.out.println(value);
        if(value!=null) return (String) value.get();

        String name = postMapper.categoryId(id);
        if (name == null) {
            throw new RuntimeException("无此Id");
        }
        cache.put("id:"+id,name);
        return name;
    }
    ...
    //获取热门标签
    @Cacheable(cacheNames = "getTagOrder", key = "'hotTagNumber:' + #hotTagNumber")
...
//
    ...
    //获取帖子列表
    @Cacheable(cacheNames = "listPost", key = "targetClass+#postSearchRequestDto.toString() +#pagination.toString()+ #token")
...
//            }
    ...
    //获取回复列表
    public Map<String, Object> listReply(PostSearchRequestDto postSearchRequestDto, Pagination pagination, int type, String token) {
        Post post = new Post();
        BeanUtils.copyProperties(postSearchRequestDto, post);
