...
//            "<if test='type !=0'> GROUP BY id HAVING view_count+reply_count*3+like_count*5+favorite_count*10 >=1 ORDER BY view_count+reply_count*3+like_count*5+favorite_count*10 </if> " +
    ...
    @Select("<script>" +
            ...
            //倒序输出5条记录
            "ORDER BY id "+
            "DESC LIMIT #{offset},#{pagination.limit} " +
            "</script>")
    List<Post>findReplyList(@Param("parentId")Long parentId,@Param("pagination") Pagination pagination);

    @Select("<script>" +
            "SELECT COUNT(*) " +
            "FROM post " +
            "<where>" +
            " AND parent_id=#{parentId} " +//父类Id，区别是帖子还是评论，不能省略
            "</where>" +
            //倒序输出5条记录
            "</script>")
    List<Post>findReplyCount(@Param("parentId")Integer parentId,@Param("pagination") Pagination pagination);
}